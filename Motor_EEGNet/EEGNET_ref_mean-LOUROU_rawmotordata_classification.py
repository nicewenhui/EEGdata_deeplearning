# -*- coding: utf-8 -*-
"""
Created on Fri Nov 29 10:05:39 2019
...
def get_data_from(file):
    ...
    # data_ear = [] #for left/right ear
     ...
     left_data = raw.copy().pick_channels(left_chans).get_data()
     right_data = raw.copy().pick_channels(right_chans).get_data()
    
     #reference to each side
     left_data = left_data - (left_data[-1,:]+right_data[-1,:])*0.5
     right_data = right_data - (left_data[-1,:]+right_data[-1,:])*0.5
    

     # Create an order 4 bandpass butterworth filter.
     left_data_filtered = butter_bandpass_filter(left_data, 4, 38, fs, order=4)
     right_data_filtered = butter_bandpass_filter(right_data, 4, 38, fs, order=4)

    
     #keep only 3 chans
     left_data_filtered = left_data_filtered[0:3,:]
     right_data_filtered = right_data_filtered[0:3,:]
    


     data_final = np.vstack([left_data_filtered, right_data_filtered])
#    normalize data
     #electrode-wise exponential moving standardizatio
     m = np.mean(data_final[:,0:1000], axis = 1) #m0 is the first 1000 datapoints mean values
     v = np.var(data_final[:,0:1000],axis = 1) #v0 is the first 1000 datapoints variance values
     normalize_data = np.zeros((data_final.shape[0], data_final.shape[1]))

     for i in range(1000,data_final.shape[1]):
         m = 0.001*data_final[:,i] + 0.999*m
         v = 0.001*((data_final[:,i]-m)**2) + 0.999*v
         normalize_data[:,i] = (data_final[:,i]-m)/np.sqrt(v)
     ...
     for i in range(trial_num):
         if desc[i] == '1':
             labels.append(0)
             events_start = events[i,0]-1*fs
             events_end = events[i,0]+2*fs
             data_trial.append(normalize_data[:,events_start:events_end]) 
